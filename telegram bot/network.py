...
def open_cv(update, context, e_camera):
    ...
    if info=="a":
        ...
        while True:
            ...
            for(x,y,w,h) in faces:
                ...
                if confidence < 100:
                    try:
                        ...
                        print ("Лицо человека не удалось распознать")
                else:
                    id = "unknown"
                    confidence = "  {0}%".format(round(100 - confidence))
            ...
            #если за определённое время не получилось распознать лицо, остановить выполнение
            print(time.time())
            if (int(time.time())//1)-(first_time//1)>10:
                context.bot.send_message(chat_id=update.effective_chat.id, text="Время выполнения функции вышло, человек не был распознан. Для повтоной попытки нажмите кнопку Распознать человека", reply_markup=ReplyKeyboardMarkup(admin_keyboard, one_time_keyboard=True))        
                break
