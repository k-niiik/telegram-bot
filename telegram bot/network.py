...
def open_cv(update, context, e_camera):
    '''Результат функции - определены: область с наличием лица, имя человека и % совпадения'''
        ...
        while True:
            ...
            for(x,y,w,h) in faces:
                ...
                if confidence < 100:
                    ...
                    context.bot.send_photo(chat_id=update.effective_chat.id, photo=open("photo.png", "rb") )
...
def get_video_file(update,context, e6, e_camera):
    ...
    if info=="a":        
        net=cv2.dnn.readNet("MobileNetSSD_deploy.prototxt", "MobileNetSSD_deploy.caffemodel") #чтение предварительно обученной модели и конфинурационного файла для распознавания образа человека
        ...
        context.bot.send_message(chat_id=update.effective_chat.id, text="Началась запись видео")
        context.bot.send_message(chat_id=update.effective_chat.id, text="Для остановки видео нажмите кнопку", reply_markup=ReplyKeyboardMarkup(stop_video_keyboard, one_time_keyboard=True))
        while True:
            e_camera.wait()
            cam = cv2.VideoCapture(0)
            ret, img =cam.read() 
        ...
        cam.release()
        cv2.destroyAllWindows()
        context.bot.send_message(chat_id=update.effective_chat.id, text="Запись видео закончилась.", reply_markup=ReplyKeyboardMarkup(admin_keyboard, one_time_keyboard=True))
        com = cv2.VideoCapture(file_name) #Имя файла можно передать остальным функциям для работы с файлом
        totalNumFrames=com.get(cv2.CAP_PROP_FRAME_COUNT)
        print (totalNumFrames)
        if totalNumFrames<=4500:
            context.bot.send_video(chat_id=update.effective_chat.id, video=open(file_name, "rb"))  #отправление получившегося видео пользователю
